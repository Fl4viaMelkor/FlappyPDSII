...
class Forma_Geometrica_Fechada {
  public:
    virtual bool noInterior(
      coordenadas p) = 0; // retorna verdadeiro se o ponto p está no interior (mas não na extremidade) da figura
    virtual bool noPerimetro(
      coordenadas p) = 0; // retorna verdadeiro se o ponto p está no perimetro (mas não na extremidade) da figura
};
