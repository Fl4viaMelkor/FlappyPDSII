#include "../../include/interface/TelaFimdejogo.hpp" 

// Construtor
TelaFimdejogo::TelaFimdejogo(int pontuacao, int largura_tela_jogo, int altura_tela_jogo) :
    ...
    // Define as cores que serão usadas no método draw()
...
void TelaFimdejogo::draw() {
    ...
    if (fonte_texto) {
        ...
        al_draw_text(fonte_texto, cor_texto_pontuacao, centro_x, pos_y_pontuacao, ALLEGRO_ALIGN_CENTER, texto_pontuacao_str.c_str());

        
        al_draw_text(fonte_texto, cor_texto_opcoes, centro_x, pos_y_opcao_reiniciar, ALLEGRO_ALIGN_CENTER, "Pressione R para Reiniciar");
        al_draw_text(fonte_texto, cor_texto_opcoes, centro_x, pos_y_opcao_menu, ALLEGRO_ALIGN_CENTER, "Pressione M para Menu Principal");
        al_draw_text(fonte_texto, cor_texto_opcoes, centro_x, pos_y_opcao_sair, ALLEGRO_ALIGN_CENTER, "Pressione ESC para Sair");
    }

}

// step: Processa inputs (eventos do Allegro)
void TelaFimdejogo::step(ALLEGRO_EVENT& evento) {
...
EstadoProximaTela TelaFimdejogo::getProximaTelaEstado() const {
    return proxima_tela_estado;
}

// Reseta o estado para NENHUM.
void TelaFimdejogo::resetProximaTelaEstado() {
    proxima_tela_estado = EstadoProximaTela::NENHUM;
}