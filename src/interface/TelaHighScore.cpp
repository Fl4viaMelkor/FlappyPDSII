  ...
  , altura_tela(altura)
{
    ...
    if (!fonte_titulo || !fonte_scores)
        throw std::runtime_error("Falha ao carregar fontes para TelaHighScore");
}
...
void TelaHighScores::draw()
{
    ...
    // texto
    al_draw_text(fonte_titulo, al_map_rgb(255, 215, 0), centro_x, altura_tela * 0.05f, ALLEGRO_ALIGN_CENTER,
                 "RECORDES");

    std::string scores_formatados = logger.listar_dados_ordenados(": ", "\n", "\t");
    scores_formatados = scores_formatados.substr(0, scores_formatados.size() - 2);
    scores_formatados += "\n";

    if (scores_formatados.empty()) {
        al_draw_text(fonte_scores, al_map_rgb(255, 255, 255), centro_x, altura_tela * 0.5f, ALLEGRO_ALIGN_CENTER,
                     "Nenhum recorde ainda!");
    }
    else {

        al_draw_multiline_text(fonte_scores, al_map_rgb(255, 255, 255), centro_x,
                               altura_tela * 0.25f,       // Posição inicial do texto
    ...
    al_draw_text(fonte_scores, al_map_rgb(200, 200, 200), centro_x, altura_tela * 0.9f, ALLEGRO_ALIGN_CENTER,
...
EstadoProximaTela TelaHighScores::getProximaTelaEstado() const { return proxima_tela_estado; }
void TelaHighScores::resetProximaTelaEstado() { proxima_tela_estado = EstadoProximaTela::NENHUM; }
