...
Cano::Cano(float x, float largura, float abertura, float altura_tela, ALLEGRO_COLOR cor, float espessura,
  ...
  : RetanguloHitboxAbstract({ x, 0 }, largura, altura_tela)
  , // Hitbox configurada depois
  m_x(x)
  , m_largura(largura)
  , m_espaco(abertura)
  , m_color(cor)
...
void Cano::onCollision() { std::cout << "Cano colidido!\n"; }

bool Cano::colisao(coordenadas p) { return noInterior(p) || noPerimetro(p); }
