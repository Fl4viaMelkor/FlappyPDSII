...
Cano::Cano(float x, float largura, float abertura, float altura_tela,
    ...
    : RetanguloHitbox({x, 0}, largura, altura_tela),
      ...
      m_largura(largura),
      m_espaco(abertura),
      m_y_topo(0), // será ajustado logo após
...
void Cano::onCollision() {
    std::cout << "Cano colidido!" << std::endl;
}

bool Cano::colisao(coordenadas p) {
    return hitboxTopo.noInterior(p) || hitboxTopo.noPerimetro(p) ||
           hitboxBaixo.noInterior(p) || hitboxBaixo.noPerimetro(p);
}

bool Cano::noInterior(const coordenadas &p) const {
    return hitboxTopo.noInterior(p) || hitboxBaixo.noInterior(p);
}

bool Cano::noPerimetro(const coordenadas &p) const {
    return hitboxTopo.noPerimetro(p) || hitboxBaixo.noPerimetro(p);
}
